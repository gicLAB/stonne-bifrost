...
def prune_linear_layer(layer: torch.nn.Linear, index: torch.LongTensor, simulation_file, tiles_path, sparsity_ratio, dim: int = 0) -> torch.nn.Linear:
    """
    ...
    new_layer = nn.SimulatedLinear(new_size[1], new_size[0], simulation_file, tile_file, sparsity_ratio, bias=layer.bias is not None).to(layer.weight.device)
